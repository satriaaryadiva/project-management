...
export default function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    const productName = process.env.NEXT_PUBLIC_PRODUCTNAME || "BasicSass";

    // BASE NAVIGATION (All Users)
    const baseNavigation = [
        { name: 'Dashboard', href: '/app', icon: Home },
        { name: 'Projects', href: '/app/projects', icon: Briefcase },
        //{ name: 'My Tasks', href: '/app/table', icon: LucideListTodo }, // Keeping generic name for now
    ...
    if (userRole === 'admin') {
        // Insert Admin Links (e.g., after Dashboard)
        navigation = [
        ...
        // Manager specific modifications if any
        // currently same as member but with more permissions inside pages
    }
        ...
        <div className="min-h-screen bg-gray-100">
            ...
            <div className={`fixed inset-y-0 left-0 w-64 bg-white shadow-lg transform transition-transform duration-200 ease-in-out z-30 
                ...
                <div className="h-16 flex items-center justify-between px-4 border-b">
                    <span className="text-xl font-semibold text-primary-600 truncate">{productName}</span>
                    <button
                        onClick={toggleSidebar}
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between h-16 bg-white shadow-sm px-4">
                    ...
                    <div className="relative ml-auto">
                        ...
                        >
                            ...
                            <span className="hidden md:block">{user?.email || 'Loading...'}</span>
                        ...
                        {isUserDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg border">
                                <div className="p-2 border-b border-gray-100">
                                    ...
                                    <p className="text-xs text-primary-600 capitalize mt-1 border border-primary-100 bg-primary-50 inline-block px-1 rounded">
                                        {userRole || 'Member'}
                                    </p>
                                </div>
                                <div className="py-1">
